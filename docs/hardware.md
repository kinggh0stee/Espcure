...
A connection-table version of the full system wiring:

**Control side (3.3 V logic) — ESP32-C6 pins:**

| ESP32-C6 pin | Connects to |
|---|---|
| GPIO21 (SDA) | SHT45 SDA **+** OLED SDA |
| GPIO22 (SCL) | SHT45 SCL **+** OLED SCL |
| GPIO5 | Fan-rail SSR `IN+` |
| GPIO18 | Peltier (TEC) SSR `IN+` |
| GPIO19 | Heater SSR `IN+` |
| GND | all 3 SSR `IN−` |
| GPIO8 | WS2812 RGB LED (on-board — no wiring) |
| GPIO9 | BOOT button (on-board — no wiring) |
| 3.3 V | SHT45 VDD **+** OLED VCC |
| GND | SHT45 GND **+** OLED GND |
| 5 V (VIN) | Buck converter (12 V→5 V) output |

**Power side (12 V) — each SSR switches the +12 V line only:**

| 12 V PSU | → SSR (`+ in` → `− out`) | → Load (positive lead) |
|---|---|---|
| +12 V | Fan-rail SSR | Fans ×3 `+` (2× TEC hot-side + heater fan) |
| +12 V | Peltier SSR | 2× TEC `+` (wired in parallel) |
| +12 V | Heater SSR | PTC element `+` |
| **−12 V** | *(common return — never touches an SSR)* | all loads `−` (TEC, fans, PTC) |
