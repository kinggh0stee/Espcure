...
- **Peltier cold-plate dehumidification** — the TEC runs continuous 0–100% duty (15 Hz ledc) driven by the self-tuning Allende cooling loop; there is no external dehumidifier relay
- **Humidity control by dew point** (self-tuning Allende: proportional + adaptive bias, Dew Point Deadband default ±0.1 °C). A VPD mode exists in the code but is `internal: true` (hidden from the web UI and HA, not user-selectable) — see the web-UI declutter note below
...
- **10-day dry program** (dew point, epoch-anchored): day 0 ramps 15.6 °C, day 1 reaches 13.9 °C, days 2–5 hold 12.2 °C, days 6–9 hold 11.1 °C, day ≥10 auto-off. Self-heals missed ticks (HA outages, reboots); falls back to SNTP time if HA disconnects
...
- Chamber Status text sensor (Cooling / Heating / Idle / Frost Guard / Dry Floor)
...
| `output.ledc` (peltier) | 15 Hz; both TECs in parallel on GPIO18. **Not a PID output** — driven by the 20 s Allende loop as continuous duty 0.0–1.0. |
| `output.ledc` (heater) | 15 Hz; PTC element on GPIO19; `heat_output` of the PID |
| `interval` (20 s) | **Allende self-tuning cooling loop** — drives the Peltier with continuous duty (0.0–1.0) proportional output + adaptive bias learning. Outputs `peltier_duty` global + `peltier_output` LEDC. Priority: frost guard → high-temp ceiling → dew-point mode → sensor fault → dry floor → Allende math. VPD control is internal/dead (entities untouched for one-line revert). |
| `interval` (60 s) | Frost-guard loop — forces Peltier off below the floor via `frost_active` global; heater keeps running. Coordinates with the 20 s Allende loop (short-circuit on frost). |
| `interval` (2 s) | Fan control + LED refresh — fan ON when a cure program is active (`dry10_program_active`) **or** `peltier_cooling` (from the 20 s Allende loop) **or** heater heating (continuous circulation during programs); then calls the `paint_status_led` script. The LED painter is **edge-triggered** via the `led_state` global and gated by `switch.status_led_enable`. |
...
| `time` (homeassistant + sntp) | Primary: HA time sync (if connected). Fallback: SNTP (internet NTP). No cron — day advancement runs every 60 s via epoch-anchored interval (restored `dry10_start_epoch`). |
| `number.*_setpoint` | User-facing setpoints exposed to HA and web UI. `dew_point_hysteresis` (renamed on UI to "Dew Point Deadband") default 0.1 °C: now serves dual roles in Allende loop — (a) gates bias adaptation (`|error| > deadband`), and (b) triggers satisfied-cutoff (force Peltier to 0 if `error < -deadband`). `restore_value: true` — existing units keep their stored value across OTA updates. |
| `number.min_chamber_temp` / `number.max_chamber_temp` | Frost floor (default 4 °C) and high-temp safety ceiling (default 27 °C) |
| `number.min_chamber_rh` | Dry floor (default 55%, range 30–60%). If chamber RH drops below this, Peltier is suspended until RH recovers 3% above the floor. The bias integrator is frozen while the floor holds. |
| `number.allende_cool_gain` | Proportional gain for the Allende cooling loop (default 0.03125, range 0.0–0.2). Scales dew-point error to duty output (°C-to-%). Increase if response is sluggish; decrease if oscillating. |
...
| `global.peltier_cooling` | bool — single source of truth for "Peltier is condensing" (float outputs can't be read back); read by the 2 s fan control loop. |
| `global.peltier_duty` | float (0.0–1.0) — mirrors the last Allende loop output written to `peltier_output` LEDC. Updated every 20 s; read by the `peltier_output_pct` sensor to expose duty as a diagnostic percentage. `restore_value: false` — no persistence needed (derived from real-time loop state). |
| `global.cool_bias` | float (0.0–3.0, restored) — Allende adaptive-bias integrator. Learned steady-state correction factor. Starts at 1.0, drifts ±0.01 every 20 s if `|error| > deadband`, clamped 0–3. Survives reboots via `restore_value: true` so the chamber doesn't re-learn on every power cycle. Read by the `cool_bias_sensor` diagnostic. Manually reset to 1.0 via the "Reset Cool Bias" button. |
| `global.dry10_start_epoch` | uint32_t (restored) — 10-Day Dry program start timestamp (epoch seconds). Set on the first valid-time tick after the program is enabled. Authority for elapsed-day computation. |
| `global.dry10_day_internal` | bool — guards the `dry10_day` on_value re-anchor against the 60 s interval's own writes. The interval sets this true immediately before writing `dry10_day`, so re-anchor only fires on genuine manual/external edits. |
| `global.dry_floor_active` | bool — set when chamber RH drops below `min_chamber_rh` floor; forces Peltier off. Mirrors `frost_active` on the humidity side. |
...
| `switch.use_vpd_control` | VPD mode toggle — now `internal: true` (hidden from web UI + HA, not selectable). VPD branch was removed from the cooling loop; VPD entities remain in the file for a one-line revert. |
| `button` (Autotune / Restart / Clear Sensor Condensation / Reset Cool Bias) | PID autotune, controller restart, sensor condensation-clear, Allende bias reset. Heater statements come from `sht_heater_on`/`sht_heater_off` substitutions — a real heater pulse on SHT31, a no-op fresh-read on SHT45 (no on-demand heater API). "Reset Cool Bias" sets `cool_bias` back to 1.0. |
| `text_sensor.chamber_status` | Human-readable operating state (Cooling / Heating / Idle / Frost Guard / Dry Floor) |
...
| `global.cure_complete` | bool (restored) — set when a program auto-completes (the 60 s day-advancement interval's completion branch), cleared on program start. Holds the LED solid green ("cured") after the program switches off. |
...
6. The Peltier (`peltier_output`, GPIO18) must have exactly **one writer** — the 20 s/60 s Allende lambdas. Never add it back as a climate `cool_output`. Whenever the Peltier output is nonzero (≥0.01 duty), the fan must be commanded ON in the same lambda (hot-side airflow); the `peltier_cooling` global tracks this state for the 2 s fan loop.
...
- **No cold-plate sensor**: There is no DS18B20. Frost protection is software-only: if `chamber_temp` drops below `min_chamber_temp` (default 4 °C), the **Peltier is forced off** until the chamber recovers 2 °C above the floor. The heat-only PID keeps running (heating aids recovery). The `frost_active` global tracks this state and short-circuits the 20 s Allende loop.
- **GPIO reference**:
  ...
  | GPIO18 | Peltier (cooling) SSR-40 DD — `ledc` 15 Hz; driven by the 20 s Allende cooling loop |
...
- **Humidity loop drives the Peltier**: the 20 s Allende loop outputs continuous duty (0.0–1.0) to chase dew point via a self-tuning proportional+adaptive-bias controller. Cooling the cold plate below the dew point condenses moisture out of the air. The loop learns the chamber's steady-state offset (cool_bias global) over time, reducing manual tuning. The `peltier_cooling` global tracks demand (float outputs can't be read back). Whenever the Peltier output is nonzero (≥ 0.01 duty), the fan is turned on in the same lambda (hot-side airflow). Loop priority: frost guard → high-temp ceiling → dew-point mode → off (VPD mode is internal/dead but code is untouched for one-line revert).
- **Cure programs**: The 10-day dry program is epoch-anchored (restored `dry10_start_epoch`). Day advancement runs every 60 s and computes elapsed days from HA time (preferred) or SNTP fallback. Self-heals missed ticks and multi-day outages (catch-up jumps directly to the correct day). If no time source is valid, days don't advance until one is available. Day counter uses `restore_value: true` — restarting ESPHome does not reset it.
- **10-day dry program** (epoch-anchored, 60 s interval): `dry10_day` counter (0–10) represents actual days elapsed since program start, not calendar days. The interval computes `elapsed = (now_timestamp - dry10_start_epoch) / 86400` and updates the dew-point setpoint: day 0 → 15.6 °C (ramp origin), day 1 → 13.9 °C (mid-ramp), days 2–5 → 12.2 °C, days 6–9 → 11.1 °C, day ≥10 → auto-off. Program start also sets temp target 17.2 °C (63 °F). Manual edits to `dry10_day` re-anchor the schedule (setting day N equivalent to restarting N days ago). The `dry10_program_status` text sensor exposes progress. The dry floor (Peltier suspension) may still hold the output off — the setpoint schedule advances regardless.
...
- **Humidity control modes**: Dew Point mode is the only user-selectable mode. A VPD mode still exists in the code (`use_vpd_control`, `vpd_setpoint`, `vpd_hysteresis`, the `vpd`/`vpd_error` sensors) but all of it is now `internal: true` — hidden from the web UI and HA, so VPD can't be enabled and the VPD branch in the 20 s Allende loop is dead-but-harmless. The mutual-exclusion (`use_dew_point_control` on_turn_on turns off `use_vpd_control`) is still wired. To bring VPD back, remove the `internal: true` flags (and restore the `web_server` sorting keys + the HA dashboard cards). Both modes drive only the Peltier — no humidifier/dehumidifier relay. RH-only control was removed (RH without temperature is meaningless).
