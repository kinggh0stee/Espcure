...
| `script.paint_status_led` | Shared LED painter = **cure-progress**: idle purple@0.5, active program fades blue→green (`rgb(0, day/total, 1-day/total)`; total 10 for dry, 8 for Cannatrol), `cure_complete` → solid green, frost → white blink (priority override). Called by the 2 s interval, instantly by the `status_led_enable` switch and the day-counter `on_value`, and edge-triggered via `led_state` (key encodes program+day) so the Frost Blink pulse isn't restarted. |
...
- **Temp targets differ by program**: the 10-Day Dry program holds temp target 17.2 °C (63 °F); the Cannatrol 4+4 program sets 20 °C. Dew-point setpoints drive the Peltier in all cases.
