...
### Fixed
- **10-Day Dry program no longer silently stalls and over-dries when disconnected from HA at midnight.** The old midnight cron (now deleted) required HA connection to fire; if the device was offline at midnight, the day didn't advance and the chamber sat at the same dew-point setpoint (often a hold value like 11.1 °C) until the next HA reconnect — causing over-drying. The new epoch-anchored 60 s interval makes HA optional: day advancement works as long as *any* time source is valid (HA preferred, SNTP fallback).

### Removed
- **Midnight cron for 10-day dry step.** Replaced by the 60 s epoch-anchored interval (above). The `time.on_time` (cron) entry is gone; two `time:` platforms (homeassistant + sntp) now handle time resolution without a cron trigger.
