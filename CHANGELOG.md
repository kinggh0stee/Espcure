...
### Added
- **Fahrenheit display toggle** — `Fahrenheit Display` switch (default OFF = Celsius). When ON, the OLED display and the composed text readouts (Humidity Control Mode, and the OLED temp/dew-point/setpoint/frost-floor/DP-error lines) render in °F; OFF keeps everything in °C. The numeric web-UI/HA sensors keep their fixed units — both °C and °F sensors already exist (`chamber_temp`/`chamber_temp_f`, `dew_point`/`dew_point_f`), since ESPHome can't change a sensor's unit at runtime. Persists across reboots; toggling forces an immediate OLED refresh.
